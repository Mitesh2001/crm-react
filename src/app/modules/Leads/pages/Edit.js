...
function Edit() {
  ...
  const [userFlag, setUserFlag] = React.useState(true);
  const [productFlag, setProductFlag] = React.useState(true);
  ...
  const getInfo = React.useCallback(() => {
      Actions.info(id).then((response) => {
        if (response.status === "SUCCESS") {
          ...
          setSelectedIndustry(response.data.lead.industry_id);
          if (response.data.lead && response.data.lead.country) {
            setSelectedCountry(response.data.lead.country.country_id);
          }
          setEmail(response.data.lead.email);
          setSelectedState(response.data.lead.state_id);
          setSelectedCity(response.data.lead.city);
          setSelectedPostCode(response.data.lead.postcode);
          getStates(response.data.lead.country.country_id);
          getCities(response.data.lead.state_id);
          getPostCodes(response.data.lead.city);
        } else {
  ...
  // if(notAllowed){
  //   addToast("You are not allowed to edit lead", {
  //     appearance:"error",
  //     autoDismiss: true,
  //   });
  ...
  if (
    product.length > 0 &&
    info?.intrested_product &&
    info?.intrested_product.length > 0 &&
    productFlag
  ) {
    setOption(
      product.filter(
        (o1) =>
          info?.intrested_product &&
          info?.intrested_product.some((o2) => o1.id === o2.id)
      )
    );
  ...
  if (
    ...
    setUseroption(
      users.filter(
        (o1) =>
          info?.assign_user &&
          info?.assign_user.some((o2) => o1.id === o2.user_id)
      )
    );
    setUserFlag(false);
  }
  ...
  return (
    <div className="add-edit-lead">
      <Form noValidate validated={validated} onSubmit={handleSubmit}>
                  ...
                  >
                    ...
                    {leadStatusArray.map((i) => (
                      <option
                        ...
                        key={"ls" + i.id}
                        selected={
                          info?.lead_status && info.lead_status?.id === i.id
                        }
                      >
                        {i.title}
                      </option>
          ...
          <Card.Body className="p-3">
            <div className="row">
              ...
              <div className="col-md-3">
                <Form.Group>
                  ...
                  <Select
                    ...
                    // selected={info?.industry_id === option.id}
                  />
              ...
              <div className="col-md-4" style={{ display: "flex" }}>
                <Form.Group>
                  <Form.Label>Change Company Logo</Form.Label>
                ...
                {(companyLogo?.base64 || picture) && (
                  <div className="col-md-1">
                    <Image
                      style={{ height: "60px", marginLeft: "-80px" }}
                      src={companyLogo?.base64 || picture}
                    />
                  </div>
                )}
              </div>
        ...
        <Card className="rkcrm-card mb-5">
          <Card.Header className="p-3">Primary Person Information</Card.Header>
          <Card.Body className="p-3">
            <div className="row">
              <div className="col-md-4">
                <Form.Group>
                  ...
                  <Form.Control
                    name="customer_name"
                    defaultValue={info?.customer_name}
                    size="sm"
                    type="text"
                    autoComplete="off"
              ...
              <div className="col-md-6">
                <Form.Group>
                  ...
                  <Form.Control
                    ...
                    defaultValue={info?.special_instructions}
                    size="sm"
                    as="textarea"
                    autoComplete="off"
