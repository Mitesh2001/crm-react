...
function List() {
  ...
  const columns = [
    ...
    //           user.user_permission.State &&
    //           user.user_permission.State.map((item) => {
    //             return (
    //               item.name === "Edit" && (
    //                 <OverlayTrigger
    //                   placement="top"
    //                   overlay={<Tooltip>Edit State</Tooltip>}
    //                 >
    //                   <Link
    //                     to={"/master/states/" + row.state + "/edit"}
    ...
    //                 <OverlayTrigger
    //                   placement="top"
    //                   overlay={<Tooltip>Delete State</Tooltip>}
    //                 >
    //                   <Link
    //                     to='#'
    ...
    Actions.list({ ...queryParams, page: currentPage, size: perPage }).then(
      (response) => {
        if (response.status === "SUCCESS") {
          ...
          setItems(response.data.states);
          setRecords(response.data.totalRecord);
          const fromPage = (currentPage - 1) * perPage + 1;
          const toPage =
            (currentPage - 1) * perPage + response.data.states.length;
          setFrom(fromPage);
          setTo(toPage);
        }
        setLoading(false);
        ...
        <Card.Body>
          {/* <div className="row mb-3">
                        ...
                        <div className="col-md-6 text-right">
                            <Link to="/master/states/add" className="rk-add-btn" ><AddIcon />Add State</Link>
                        </div>
                    </div> */}
