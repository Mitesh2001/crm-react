...
function Activities({ lead }) {
  ...
  suhbeader.setTitle("Assign Log");
  const { id } = useParams();
  const [info, setInfo] = React.useState([]);
  const getData = React.useCallback(() => {
    Actions.loginfo({ assign_id: id, type: 2 }).then((response) => {
        ...
        {info.map((log, index) => {
          return (
            <>
              {index === 0 ? (
                <ListGroup.Item>
                  {log.contact_created_by && (
                    <>
                      {" "}
                      &#8226; Contact created at {timeUtil(log.created_at)} PM
                      By {log.contact_created_by.name}
                    </>
                  )}
                </ListGroup.Item>
              ) : null}
              {log.get_user_data && (
                <>
                  {" "}
                  <ListGroup.Item>
                    &#8226; Contact Assigned to {log.get_user_data.name}
