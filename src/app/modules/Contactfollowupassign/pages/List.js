...
export default React.memo(function({ contact }) {
  ...
  suhbeader.setTitle("Followup Assign");
      ...
      formatter: (rowContent, row) => {
        return (
          <div className="actions-btns">
            ...
            >
              <Link
                to={"/contacts/" + row.follow_up_id + "/show"}
  ...
  const getData = React.useCallback(
    (event) => {
      ...
      formDataObj.follow_up_id = contact;
      // formDataObj.type = 1;
    ...
    [queryParams, currentPage, contact, perPage]
  );
  ...
  const handleSubmit = (event) => {
    ...
    if (form.checkValidity() === true) {
      ...
      Actions.assignContact(assignParameters).then((response) => {
        setLoading(false);
        console.log(response.message);
        if (response.message === "The follow up id field is required.") {
          addToast("PLease Select Contact to Assign", {
            appearance: "error",
            autoDismiss: true,
          });
        } else {
  ...
  return (
    <>
      <div className="rk-sub-header mb-5">
        Assign user to contact's Follow Up
      </div>
              ...
              {!loading && records > 0 && (
                <>
                  ...
                  <div className="row" style={{ marginLeft: "-9px" }}>
                    {user.user_permission.ContactFollowup ? (
